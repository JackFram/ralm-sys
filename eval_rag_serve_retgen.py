...
def evaluate_logprob_with_retrieved_docs(
    ...
    while input_ids.shape[1] - query_len < max_new_token_num and tokenizer.eos_token_id not in input_ids[0, 1:]:
        ...
        with torch.no_grad():
            ...
            for i in range(spec_step):
                ...
                # output = input_ids
                output = model.generate(input_ids, do_sample=False, max_new_tokens=cur_spec_token_num)
                infer_time += 1
                spec_stride_list.append(cur_spec_token_num)
                input_ids = output[[0], query_start_idx:]
                if iter_retgen:
                    prev_gen_length = cur_spec_token_num
                iter_retgen = not iter_retgen
                ...
                if not iter_retgen:
                    input_ids = input_ids[[0], :-prev_gen_length]
                    spec_stride_list[-1] -= prev_gen_length
    ...
    # print(retrieval_latency, inference_latency)
    # print(ret_time, infer_time)
    #
    # exit(0)
